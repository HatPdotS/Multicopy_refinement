...
from multicopy_refinement.utils import TensorDict, ModuleReference
from multicopy_refinement.debug_utils import DebugMixin

class SolventModel(DebugMixin, nn.Module):
    def __init__(self, scaler, radius=1.1, k_solvent=1.1, b_solvent=50.0, erosion_radius=0.9, 
                 transition=None, optimize_phase=False, initial_phase_offset=0.0, verbose=1,float_type=torch.float32,device=torch.device('cpu')):
        """
        ...
        self.log_k_solvent = nn.Parameter(torch.log(k_solvent) * torch.ones_like(self.scaler.log_scale))
        self.b_solvent = nn.Parameter(b_solvent * torch.ones_like(self.scaler.log_scale))
    ...
    def forward(self, hkl, update_fsol=False, F_protein=None):
        ...
        b_solvent = self.b_solvent[self.scaler.bins]

        k_solvent = torch.exp(self.log_k_solvent[self.scaler.bins])
