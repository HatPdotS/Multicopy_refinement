...
#SBATCH -c 16
#SBATCH -o /das/work/p17/p17490/Peter/Library/multicopy_refinement/tests_manual/benchmark_refining/separate_step_LBFGS/screen_restraint_weights/weight_screening_adp_2.out
#SBATCH -p week
#SBATCH -t 2-00:00:00 
...
def screen_weights(mtz, pdb, outdir, n_cycles=10):
    ...
    restraints_weights = np.logspace(np.log10(0.1), np.log10(10.0), 20)
    adp_weight = 2
    ...
    print(f"ADP weight: {adp_weight}")
    print(f"Number of cycles per run: {n_cycles}")
    print(f"Total combinations: {len(restraints_weights)}")
    print("=" * 80)
    ...
    for i, restraints_wt in enumerate(restraints_weights):
            combo_id = i
            total_combos = len(restraints_weights)
            
            print("\n" + "=" * 80)
            print(f"Combination {combo_id}/{total_combos}")
            print(f"Restraints weight: {restraints_wt:.4f}")
            print("=" * 80)
            ...
            # Save model
            model_file = f"{outdir}/screened_model_r{restraints_wt:.3f}_adp_{adp_weight:.3f}.pdb"
...
if __name__ == "__main__":
    ...
    outdir = '/das/work/p17/p17490/Peter/Library/multicopy_refinement/tests_manual/benchmark_refining/separate_step_LBFGS/screen_restraint_weights'
